...
# Load the data                                                     #sim_0.01-2-0.05-nsyn_boot.0.subsampled.2d.fs
data = dadi.Spectrum.from_file('../results/SFS/by_dadi/Bootstraps/sim_'+migration+'-'+possel+'-'+change+'-'+site+'_boot.'+iteration+'.subsampled.2d.fs')
ns = data.sample_sizes

### TEST 4 DIFFERENT STRENGTHS OF POSITIVE SELECTION and two pb
for new_possel in [2,20,200,0]: #[2,20,200,0] 
    print(possel)
    print(new_possel)
    ...
    # params = [0.3, 100, ppos_wild, gamma_pos, 0, 0, 0.01]
    ...
    try:
    	fid = open('../results/dadi_outputs/Bootstraps/sim_'+migration+'-'+possel+'-'+change+'-'+site+'_boot.'+iteration+'_fits_with_Sb='+str(new_possel)+'_two_pb.txt','a')
    except:
    	fid = open('../results/dadi_outputs/Bootstraps/sim_'+migration+'-'+possel+'-'+change+'-'+site+'_boot.'+iteration+'_fits_with_Sb='+str(new_possel)+'_two_pb.txt','w')
...
for new_possel in [2,20,200,0]: #[2,20,200,0] 
    ...
    # that you don't overwrite other results
    try:
        fid = open('../results/dadi_outputs/Bootstraps/sim_'+migration+'-'+possel+'-'+change+'-'+site+'_boot.'+iteration+'_fits_with_Sb='+str(new_possel)+'_one_pb.txt','a')
    except:
        fid = open('../results/dadi_outputs/Bootstraps/sim_'+migration+'-'+possel+'-'+change+'-'+site+'_boot.'+iteration+'_fits_with_Sb='+str(new_possel)+'_one_pb.txt','w')
