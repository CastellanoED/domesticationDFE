source("./polyDFEv2.0/postprocessing.R")
...
dfe_wild$population = "Wild" #labels are wrong!
dfe_dom$population = "Domestic"

#getDiscretizedDFE(est[[2]])
# change the binning
#getDiscretizedDFE(est[[3]], sRanges = c(-10, -1, 0, 1, 10))
...
#avg_dfe = rowSums(sapply(1:length(est), function(i) aic_weights[i, "weight"] * dfe[i, ]), na.rm = TRUE)
#print(avg_dfe)
#barplot(avg_dfe)
#avg_dfe2 = data.frame(rbind(avg_dfe))
#avg_dfe2$simu = args[1]
#avg_dfe2$rep = args[2]
#avg_dfe2$mod = "1_10_2_20"

#write.table(avg_dfe2,file="../output/avg_dfe_replicate_allmodels.txt",row.names = F, col.names = F, quote = F, append = TRUE)
...
#avg_shape = sum(aic_weights$weight * shape)
#avg_p_b = sum(aic_weights$weight * p_b)
#avg_S_b = sum(aic_weights$weight * S_b)
#avg_S_d = sum(aic_weights$weight * S_d)
...
#output2 = cbind(avg_shape,avg_p_b,avg_S_b,avg_S_d,args[1],args[2],"1_10_2_20")  
#write.table(output2,file="../output/avg_parameters_replicates_allmodels.txt",row.names = F, col.names = F, quote = F, append = TRUE)
